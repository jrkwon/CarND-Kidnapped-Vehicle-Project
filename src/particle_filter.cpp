	...
	for (int i = 0; i < num_particles; i++) {
		...
		for (uint j = 0; j < map_landmarks.landmark_list.size(); j++) {
			if ((fabs(particles[i].x - map_landmarks.landmark_list[j].x_f) <= sensor_range)
				...
				std::cout << "predicted: " << map_landmarks.landmark_list[j].id_i << ", " <<  
							map_landmarks.landmark_list[j].x_f << ", " << 
							map_landmarks.landmark_list[j].y_f << std::endl;
			}
		}
		...
		for (uint j = 0; j < trans_observations.size(); j++) {
			...
			double gaussian_prob 
				= exp(-(
						(pow((trans_observations[j].x - pred_x), 2)/(2.0*pow(std_landmark[0], 2)))
						+ (pow((trans_observations[j].y - pred_y), 2)/(2.0*pow(std_landmark[1], 2)))));
